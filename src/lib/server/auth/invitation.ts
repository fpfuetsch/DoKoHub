...
if (rawInvitationSecret === DEFAULT_INVITATION_JWT_SECRET) {
  ...
  console.warn('WARNING: Using default Invitation JWT secret. Set INVITATION_JWT_SECRET in env for production.');
}
const secret = new TextEncoder().encode(rawInvitationSecret);
export const INVITATION_MAX_AGE_SECONDS = env.INVITATION_MAX_AGE_SECONDS ? parseInt(env.INVITATION_MAX_AGE_SECONDS) : 24 * 60 * 60; // default: 24 hours

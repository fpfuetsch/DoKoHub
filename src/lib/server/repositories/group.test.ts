...
describe('GroupRepository', () => {
  ...
  describe('list / getById', () => {
    ...
    it('lists only groups where user is a member (isolation)', async () => {
      ...
      expect(getOther).toEqual({ ok: false, error: 'Gruppe nicht gefunden oder keine Berechtigung.', status: 404 });
    });
  });
  ...
  describe('create', () => {
    ...
    it('creates group with principal and unique members', async () => {
      const p1 = await playerRepo.create({ displayName: 'Member A', authProvider: AuthProvider.Local, authProviderId: null });
      const p2 = await playerRepo.create({ displayName: 'Member B', authProvider: AuthProvider.Local, authProviderId: null });
  ...
  describe('leave', () => {
    it('deletes group when last non-local user leaves', async () => {
      ...
      const lp1 = await playerRepo.create({ displayName: 'Local 1', authProvider: AuthProvider.Local, authProviderId: null });
      const lp2 = await playerRepo.create({ displayName: 'Local 2', authProvider: AuthProvider.Local, authProviderId: null });
    ...
    it('removes membership when other non-local members remain', async () => {
      ...
      const nonLocal = await playerRepo.create({ displayName: 'NonLocal', authProvider: AuthProvider.Google, authProviderId: 'google|x' });
      ...
      const membership = await db
        ...
        .where(and(eq(GroupMemberTable.groupId, g.value.id), eq(GroupMemberTable.playerId, principalId)));
  ...
  describe('delete', () => {
    ...
    it('deletes group and local players', async () => {
      ...
      const localP = await playerRepo.create({ displayName: 'Local X', authProvider: AuthProvider.Local, authProviderId: null });
      await repo.addMember(g.value.id, localP.id);

      // add a non-local player to ensure they are not deleted
      const nonLocal = await playerRepo.create({ displayName: 'NonLocal', authProvider: AuthProvider.Google, authProviderId: 'google|y' });
      await repo.addMember(g.value.id, nonLocal.id);
      ...
      // non-local remains
      const playersNonLocal = await db.select().from(PlayerTable).where(eq(PlayerTable.id, nonLocal.id));

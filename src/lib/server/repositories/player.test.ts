...
describe('PlayerRepository', () => {
  ...
  describe('basic getters', () => {
    ...
    it('create and fetch by provider', async () => {
      const p = await repo.create({ displayName: 'A', authProvider: AuthProvider.Google, authProviderId: 'g|1' });
  ...
  describe('createLocal', () => {
    it('requires membership', async () => {
      ...
      const other = await otherBase.create({ displayName: 'Other', authProvider: AuthProvider.Google, authProviderId: 'g|x' });
    ...
    it('creates local player and membership', async () => {
      ...
      if (res.ok) {
        ...
        const membership = await db.select().from(GroupMemberTable).where(and(eq(GroupMemberTable.groupId, groupId), eq(GroupMemberTable.playerId, res.value.id)));
  ...
  describe('deleteLocal', () => {
    it('requires membership', async () => {
      ...
      const otherBase = new PlayerRepository();
      const other = await otherBase.create({ displayName: 'Other', authProvider: AuthProvider.Google, authProviderId: 'g|y' });
    ...
    it('only local players can be deleted', async () => {
      const nonLocal = await repo.create({ displayName: 'NonLocal', authProvider: AuthProvider.Google, authProviderId: 'g|z' });
      const res = await repo.deleteLocal(nonLocal.id, groupId);
      expect(res).toEqual({ ok: false, error: 'Nur lokale Spieler können bearbeitet werden.', status: 400 });
    });
    ...
    it('prevents deletion when participations exist', async () => {
      ...
      const p2 = await repo.create({ displayName: 'P2', authProvider: AuthProvider.Local, authProviderId: null });
      const p3 = await repo.create({ displayName: 'P3', authProvider: AuthProvider.Local, authProviderId: null });
      const p4 = await repo.create({ displayName: 'P4', authProvider: AuthProvider.Local, authProviderId: null });
      const createGame = await gameRepo.create(groupId, 24, true, [localRes.value.id, p2.id, p3.id, p4.id]);
  ...
  describe('renameLocal', () => {
    it('requires membership', async () => {
      const otherBase = new PlayerRepository();
      const other = await otherBase.create({ displayName: 'Other', authProvider: AuthProvider.Google, authProviderId: 'g|a' });
      const otherGroupRepo = new GroupRepository(other.id);
      const g = await otherGroupRepo.create('Other Group');
    ...
    it('only local players can be renamed', async () => {
      const nonLocal = await repo.create({ displayName: 'NonLocal', authProvider: AuthProvider.Google, authProviderId: 'g|b' });
  ...
  describe('takeoverLocalPlayer', () => {
    it('requires membership of group', async () => {
      const localInOther = await new PlayerRepository(principalId).create({ displayName: 'Local S', authProvider: AuthProvider.Local, authProviderId: null });
      // make a foreign group with other user
      const other = await new PlayerRepository().create({ displayName: 'Other', authProvider: AuthProvider.Google, authProviderId: 'g|c' });
    ...
    it('validates local and target types', async () => {
      const notLocal = await repo.create({ displayName: 'NonLocal', authProvider: AuthProvider.Google, authProviderId: 'g|t' });
      const res1 = await repo.takeoverLocalPlayer(notLocal.id, groupId);
      expect(res1).toEqual({ ok: false, error: 'Quellspieler ist kein lokaler Spieler.' , status: 400});

      // target must be non-local: use a local principal
      const localPrincipal = await new PlayerRepository().create({ displayName: 'Local Principal', authProvider: AuthProvider.Local, authProviderId: null });
      ...
      expect(res2).toEqual({ ok: false, error: 'Zielspieler darf kein lokaler Spieler sein.', status: 400 });
    });
    ...
    it('fails when both participated in the same game', async () => {
      ...
      const game = await db.insert(GameTable).values({ groupId, maxRoundCount: 24, withMandatorySolos: true }).returning();
      const gameId = game[0].id as string;
      await db.insert(GameParticipantTable).values({ gameId, playerId: localRes.value.id, seatPosition: 0 });
      await db.insert(GameParticipantTable).values({ gameId, playerId: principalId, seatPosition: 1 });

      const res = await repo.takeoverLocalPlayer(localRes.value.id, groupId);
      expect(res).toEqual({ ok: false, error: 'Du hast bereits mit dem lokalen Spieler an einem Spiel teilgenommen. Übernahme nicht möglich.', status: 400 });
    });
    ...
    it('succeeds and transfers memberships and references', async () => {
      ...
      const game = await db.insert(GameTable).values({ groupId, maxRoundCount: 24, withMandatorySolos: false }).returning();
      const gameId = game[0].id as string;
      await db.insert(GameParticipantTable).values({ gameId, playerId: localRes.value.id, seatPosition: 0 });
      ...
      const parts = await db.select().from(GameParticipantTable).where(and(eq(GameParticipantTable.gameId, gameId), eq(GameParticipantTable.playerId, principalId)));
      expect(parts.length).toBe(1);

      // no duplicate membership remains
      const members = await db.select().from(GroupMemberTable).where(and(eq(GroupMemberTable.groupId, groupId), eq(GroupMemberTable.playerId, principalId)));

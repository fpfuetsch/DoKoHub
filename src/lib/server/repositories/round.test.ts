...
function makeNormalRoundDraft(p0: string, p1: string, p2: string, p3: string): RoundData {
  return {
    ...
    eyesRe: 121,
    participants: [
      { playerId: p0, team: Team.RE, calls: [{ playerId: p0, callType: CallType.RE }], bonuses: [] },
      { playerId: p2, team: Team.RE, calls: [], bonuses: [] },
      { playerId: p1, team: Team.KONTRA, calls: [{ playerId: p1, callType: CallType.KONTRA }], bonuses: [] },
...
describe('RoundRepository', () => {
  ...
  beforeEach(async () => {
    ...
    const principal = await playerRepo.create({ displayName: 'Principal', authProvider: AuthProvider.Google, authProviderId: 'google|principal' });
    ...
    playerIds = [];
    for (let i = 0; i < 5; i++) {
      const p = await playerRepo.create({ displayName: `P${i+1}` , authProvider: AuthProvider.Local, authProviderId: null });
      playerIds.push(p.id);
    }
  ...
  describe('getRoundsForGame', () => {
    it('forbids access for non-members', async () => {
      const other = await playerRepo.create({ displayName: 'Other', authProvider: AuthProvider.Local, authProviderId: null });
  ...
  describe('addRound', () => {
    ...
    it('creates a round and persists participants, calls, bonuses and results', async () => {
      ...
      draft.participants[0].bonuses.push({ playerId: playerIds[1], bonusType: BonusType.Fuchs, count: 1 });
      const res = await roundRepo.addRound(gameId, groupId, draft);
      expect(res.ok).toBe(true);
      if (res.ok) {
        const rid = res.value.id;
        const parts = await db.select().from(GameRoundParticipantTable).where(eq(GameRoundParticipantTable.roundId, rid));
        expect(parts.length).toBe(4);
        const calls = await db.select().from(GameRoundCallTable).where(eq(GameRoundCallTable.roundId, rid));
        expect(calls.length).toBe(2);
        const bonuses = await db.select().from(GameRoundBonusTable).where(eq(GameRoundBonusTable.roundId, rid));
        expect(bonuses.length).toBe(1);
        const results = await db.select().from(GameRoundResultTable).where(eq(GameRoundResultTable.roundId, rid));
  ...
  describe('getById', () => {
    it('forbids when round not in user group', async () => {
      ...
      expect(res).toEqual({ ok: false, error: 'Runde nicht gefunden oder keine Berechtigung.', status: 404 });
    });
  ...
  describe('updateRound', () => {
    it('prevents switching Pflicht/Lust on existing rounds', async () => {
      ...
      const update: RoundData = { ...draft, id: created.value.id, roundNumber: 1, soloType: SoloType.Lust };
      const res = await roundRepo.updateRound(created.value.id, gameId, groupId, update);
      expect(res).toEqual({ ok: false, error: 'Für eine bestehende Runde kann nicht geändert werden, ob sie eine Pflicht- oder Lust-Solo-Runde ist.', status: 400 });
    });
    ...
    it('calls Round.validate and Game.validate when updating', async () => {
      ...
      const updated: RoundData = {
        ...
        participants: [
          ...
          { playerId: playerIds[2], team: Team.KONTRA, calls: [{ playerId: playerIds[2], callType: CallType.KONTRA }, { playerId: playerIds[2], callType: CallType.Keine90 }] , bonuses: [] },
          { playerId: playerIds[4], team: Team.KONTRA, calls: [], bonuses: [] }
        ]
      };

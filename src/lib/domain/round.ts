...
export class Round implements RoundData {
    ...
    static validate(round: RoundData, withMandatorySolos: boolean = false): string | null {
        ...
        const bonusesAllowed = round.type === RoundType.Normal || round.type === RoundType.HochzeitNormal;
        ...
        for (const participant of this.participants) {
            if (participant.team === team) {
                for (const bonus of participant.bonuses) {
                    if (bonus.bonusType === BonusType.Doko || bonus.bonusType === BonusType.Fuchs || bonus.bonusType === BonusType.Karlchen) {
                        points += bonus.count;
                    }
                }
            }

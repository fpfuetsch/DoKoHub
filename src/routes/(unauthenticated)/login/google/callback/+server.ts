...
export const GET: RequestHandler = async ({ cookies, url }) => {
	...
	const existing = await playerRepo.getByProvider(AuthProvider.Google, idTokenDecoded.sub as string);
	...
	if (!existing) {
		const onboardingToken = await createOnboardingToken({
			...
			suggestedDisplayName: (((idTokenDecoded.given_name as string) || '') + ' ' + ((idTokenDecoded.family_name as string) || '')).trim(),

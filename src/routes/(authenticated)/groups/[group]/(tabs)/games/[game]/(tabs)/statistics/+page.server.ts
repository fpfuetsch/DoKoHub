...
export const load: ServerLoad = async ({ locals, params }) => {
    ...
    const callTypes = [CallType.RE, CallType.KONTRA, CallType.Keine90, CallType.Keine60, CallType.Keine30, CallType.Schwarz];
    ...
    for (const round of game.rounds) {
        ...
        for (const participant of round.participants) {
            ...
            eyesTotals.set(participant.playerId, (eyesTotals.get(participant.playerId) || 0) + achievedEyes);
        ...
        for (const rp of roundPoints) {
            ...
            const lastCumulative = playerHistory.length > 0 ? playerHistory[playerHistory.length - 1].cumulativePoints : 0;
            playerHistory.push({ round: round.roundNumber, cumulativePoints: lastCumulative + rp.points });
